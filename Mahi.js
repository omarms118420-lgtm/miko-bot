...
(async () => {
        ...
        const gmailEnabled = config.notiWhenListenMqttError && config.notiWhenListenMqttError.gmail && config.notiWhenListenMqttError.gmail.enable;
        const OAuth2 = google.auth.OAuth2;
        const OAuth2_client = new OAuth2(clientId, clientSecret);
        OAuth2_client.setCredentials({ refresh_token: refreshToken });
        let accessToken;
        if (gmailEnabled && clientId && clientSecret && refreshToken) {
        try {
                accessToken = await OAuth2_client.getAccessToken();
        }
        catch (err) {
                utils.log.warn("MAIL", getText("Goat", "googleApiTokenExpired"));
        }
        }
        ...
        try {
                ...
                utils.log.warn("DRIVE", "Google Drive غير مفعّل أو بيانات الاعتماد غير صالحة، سيتم تخطّي هذه الخطوة.");
        }
        // ———————————————————— LOGIN ———————————————————— //
